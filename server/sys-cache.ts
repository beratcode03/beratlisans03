...
class SysCacheManager {
  private logs: SysCacheLog[] = [];
  private offlineQueue: SysCacheLog[] = [];
  ...
  async log(activity: Omit<SysCacheLog, 'id' | 'timestamp'>): Promise<SysCacheLog> {
    const log: SysCacheLog = {
      id: randomUUID(),
      timestamp: new Date(),
      ...activity,
    };
  ...
  private async sendToDiscord(log: SysCacheLog): Promise<void> {
  ...
  getLogs(filter?: {
    type?: SysCacheLog['type'];
    severity?: SysCacheLog['severity'];
    startDate?: Date;
    endDate?: Date;
    userId?: string;
    limit?: number;
...
export const sysCache = new SysCacheManager();
export const userMonitoring = sysCache;
